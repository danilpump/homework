...
void print(struct contacts *a, int r) {
	//¬ывод текущего списка контактов (фамили€, им€,тел.номер)                       !!!
...
int add(struct contacts *a, int r) {
	//ƒобавление контакта (об€зательные пол€: фамили€, им€, номер)                   !!!
	if (r==maxsize) {
...
void edit(struct contacts* a, int i) {
	//–едактирование выбранного контакта                                             !!!
	int sw = -1;
	do {
		printf("1-Name:          %s\n", a[i].name);
		printf("2-Surname:       %s\n", a[i].surname);
		...
		case 3:
			...
			break;
		case 4:
			printf("Enter the patronymic (you can delete this field by writing -):\n");
			scanf("%20s", &a[i].patronymic);
			if (a[i].patronymic[0] == '-')
				strcpy(a[i].patronymic, "");
			break;
		case 5:
			printf("Enter the email (you can delete this field by writing -):\n");
			scanf("%30s", &a[i].email);
			if (a[i].email[0] == '-')
				strcpy(a[i].email, "");
			break;
		case 6:
			printf("Enter the organization (you can delete this field by writing -):\n");
			scanf("%30s", &a[i].organization);
			if (a[i].organization[0] == '-')
				strcpy(a[i].organization, "");
			break;
		case 7:
			printf("Enter the position (you can delete this field by writing -):\n");
...
int view(struct contacts *a, int r) {
	//ѕросмотр выбранного контакта (сперва надо придумать метод выбора)              !!!
	int tmp;
	int sw = -1;
...
void search_sn(struct contacts *a, int r) {
	//ѕоиск контакта по фамилии или ее части                                         !!!
	char tmp[20];
...
void search_ph(struct contacts* a, int r) {
	//ѕоиск контакта по номеру телефона или его части                                !!!
	char tmp[15];
...
void base(struct contacts *a) {
	//—юда надо забить контакты самому                                               !!!
