...
void quick_sort(struct _finddata_t *c_file, int filecount) {
    for (int i = 1; i < filecount; i++) {
        for (int j = i; j > 0 && c_file[j - 1].size > c_file[j].size; j--) {
            int size = c_file[j - 1].size;
            char name[256];
            c_file[j - 1].size = c_file[j].size;
            c_file[j].size = size;
            strcpy_s(name, 256, c_file[j - 1].name);
            strcpy_s(c_file[j - 1].name, 256, c_file[j].name);
            strcpy_s(c_file[j].name, 256, name);
        }
    }
}
